		...
		<tr>
			<td valign="top">
				<div class="tableContentLeft">
					...
					<div class="navSupTitle"><a class="act" href="/knowledgeDictLibManage/knowledgeDictLibManageToAddItem" ><span>添加词条</span></a></div>
...
<!-- submit start -->
						<div class="submitBox modifyBox">
							...
							<span class="returnBnt"><a href="/knowledgeDictLibManage/knowledgeDictLibManageItemManage" ></a>
...
function submitForm(){
	...
	if(hasErrors==false){
		...
		success:function(msg){
			if(msg.exit){
				alert("in exitName");
				$("#exitName").html("<a href=\"/knowledgeDictLibManage/knowledgeDictLibManageToAddItem\">词库已存在，更新失败！");
			}
			else if(msg.errors){
				$(".restate span").html("<img src=\"${ctx}/images/loading.gif\" />");
				$(".importPrompt").html();
				$(".restate span").append("更新词条失败");
				$(".restate").css("display","block");
			}else{
				$(".restate span").html("<img src=\"${ctx}/images/loading.gif\" />");
				$(".importPrompt").html();
				$(".restate span").append("更新词条成功");
				$(".restate").css("display","block");
			}
