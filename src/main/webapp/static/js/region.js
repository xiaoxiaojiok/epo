...
function getItemVal(pageGo)
{
	...
	objParams['NewsRegion[start]'] = rangDate[0];
	objParams['NewsRegion[end]'] = rangDate[1];
...
{
	var tableHeader = '<tr><th align="center" width="35%">标题</th><th align="center" width="9%">转载量</th><th align="center" width="18%">最近转载</th><th align="center" width="9%">涉及区县</th><th align="center" width="18%">发布时间</th><th align="center" width="9%">操作</th></tr>';
	var tableContent = '';
	if (response.data && response.data.length > 0){
		for(var i=0; i<response.data.length; i++) {

			tableContent += '<td align="center" width="35%"><div class="pTitle"><a target="_blank" href="'+response.data[i].url+'" title="'+response.data[i].fullTitle+'">'+response.data[i].title+'</a></div></td>';
			tableContent += '<td align="center" width="9%">'+response.data[i].reserved+'</td>';
			tableContent += '<td align="center" width="18%">'+response.data[i].updateDate+'</td>';
			tableContent += '<td align="center" width="9%">'+response.data[i].region+'</td>';
