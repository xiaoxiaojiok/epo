	...
	function putWeibo(data){
		...
		if(data==''){
			...
			for(e in data){
					...
					weiboList+='<span class="soucre clubsoucre PeopleTimeMbnAdd">查看：<tt>'+data[e].ftorwarding+'</tt></span>';
					weiboList+='</div>';
					weiboList+='<div class="sumTime">';
					weiboList+='<span class="name PeopleTimeM PeopleTimeMbnLaiy">作者：<tt>'+data[e].author+'</tt></span>';
